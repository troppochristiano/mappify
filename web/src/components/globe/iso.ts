/**
 * ISO 3166-1 numeric → alpha-2, for the 177 countries in the world-atlas
 * topology.
 *
 * The topology identifies a country by its numeric code and carries nothing
 * else but a display name; mappify stores alpha-2, which is what Wikidata gives
 * for a place's country. Something has to join the two, and matching on the
 * name cannot: the topology calls them "W. Sahara", "Dem. Rep. Congo" and
 * "Bosnia and Herz.", none of which is what Wikidata says.
 *
 * A table rather than a dependency because it is 177 lines that will not change
 * — and the alternative packages carry translations, subdivisions and flags for
 * a lookup that is used in one place.
 *
 * N. Cyprus, Somaliland and Kosovo have no numeric code in the topology (their
 * id is undefined) and so are absent here. They are drawn by the coast layer
 * like everywhere else; they just cannot be highlighted, which is correct —
 * mappify has no alpha-2 to ask for them with either.
 */
